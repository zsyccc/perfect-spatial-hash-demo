...
struct voxelgroup {
    ...
    friend std::ostream& operator<<(std::ostream& stream,
                                    const voxelgroup& vg) {
        stream << "(";
        for (uint i = 0; i < 8; i++) stream << ", " << vg.voxels[i];
        stream << ")";
        return stream;
    }
};
...
void voxel_test() {
    ...
    std::set<uint32_t> data_b;
    for (PosInt x = 0; x < width; x++) {
        for (PosInt y = 0; y < width; y++) {
            for (PosInt z = 0; z < width; z++) {
                if (rand() % 10 == 0) {
                    // std::cout << point{x, y, z} << std::endl;
                    data.push_back(map::data_t{
                        point{x, y, z},
                        // voxel{uint16_t(x + y + z)}
                        voxel{uint16_t(x), uint16_t(y), uint16_t(z),
                              uint16_t(x + 1), uint16_t(y + 1), uint16_t(z + 1),
                              uint16_t(x + 2), uint16_t(y + 2)}});
                    data_b.insert(psh::point_to_index<d>(point{x, y, z}, width,
                                                         uint(-1)));
                    // data_b[psh::point_to_index<d>(point{x, y, z}, width,
                    //                               uint(-1))] = true;
                }
            }
        }
    ...
    auto original_data_size =
        width * width * width * (sizeof(voxel) + sizeof(point));
    ...
    std::cout << "compression factor vs dense: "
              << (float(s.memory_size()) /
                  (uint(std::pow(width, 3)) * sizeof(voxel)))
              << std::endl;
...
void game_of_life_test() {
    ...
    for (PosInt x = 0; x < width; x++) {
        for (PosInt y = 0; y < width; y++) {
            if (rand() % 20 == 0) {
                data.push_back(map::data_t{point{x, y}, pixel{true}});
                data_b[psh::point_to_index<d>(point{x, y}, width, uint(-1))] =
                    true;
            }
        }
    }
    ...
    auto finalize = [&]() {
        if (add_failed) {
            ...
            s = s.rebuild([&](size_t i) { return new_data[i]; }, queue.size(),
                          data_b);
            queue.clear();
            add_failed = false;
            num_rebuilds++;
        ...
        std::cout << num_rebuilds << " rebuilds so far, memory use: "
                  << s.memory_size() / (1024 * 1024.0f) << "mb" << std::endl
                  << std::endl;
    };
    ...
    for (uint i = 0; i < 100000000; i++) {
        ...
        for (uint j = 0; j < 20; j++) std::cout << std::endl;
