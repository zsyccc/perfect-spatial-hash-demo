...
namespace psh {
    ...
    public:
        ...
        map(const data_function& data, IndexInt n, PosInt u_bar)
            : n(n),
              ...
              m(std::pow(m_bar, d)),
              r_bar(std::ceil(std::pow(n / d, 1.0f / d)) - 1),
              u_bar(u_bar),
              u(std::pow(u_bar, d)),
              generator(time(0)) {
        ...
        map rebuild(const data_function& new_data, IndexInt new_n,
                    const std::vector<bool>& data_b) {
            ...
            return map([&](IndexInt i) { return data[i]; }, data.size(), u_bar);
        }
    ...
    private:
        ...
        struct entry {
            ...
            entry(const data_t& data, IndexInt M2)
                : contents(data.contents), k(1) {
                rehash(data, M2);
            }

            static constexpr HashInt h(const point<d, PosInt>& p, IndexInt M2,
                                       HashInt k) {
            ...
            void rehash(const point<d, PosInt>& location, IndexInt M2,
                        HashInt new_k = 1) {
                k = new_k;
                hk = h(location, M2, k);
            }
            ...
            constexpr bool equals(const point<d, PosInt>& p,
                                  IndexInt M2) const {
                return hk == h(p, M2, k);
            }
        };
        ...
        point<d, PosInt> h(const point<d, PosInt>& p,
        ...
        bool create(const data_function& data,
            ...
            for (IndexInt i = 0; i < buckets.size(); i++) {
                ...
                if (!jiggle_offsets(H_hat, H_b_hat, phi_hat, buckets[i],
                                    m_dist)) {
                    return false;
                }
            }
        ...
        bool jiggle_offsets(std::vector<entry_large>& H_hat,
            ...
            tbb::parallel_pipeline(
                ...
                tbb::make_filter<void, IndexInt>(
                    ...
                    tbb::make_filter<IndexInt, void>(
                        tbb::filter::parallel, [&, group_size](IndexInt i0) {
                            for (IndexInt i = i0; i < i0 + group_size && !found;
                                 i++) {
                                ...
                                for (auto& element : b) {
                                    ...
                                    // is invalid
                                    collision =
                                        H_b_hat[point_to_index(hash, m_bar, m)];
                                    if (collision) break;
                                }
                                ...
                                if (!collision) {
                                    ...
                                    if (!found) {
                                        // this MIGHT get overwritten more than
                                        // once, but that's okay, we only need
                                        // one to succeed, not specifically the
                                        // first
                                        found = true;
        ...
        bool hash_positions(const data_function& data,
            ...
            IndexInt domain_i_max =
                point_to_index(point<d, PosInt>::repeating(u_bar) - PosInt(1),
                               u_bar, IndexInt(-1)) +
                1;
        ...
        bool fix_k(entry_large& H_entry,
                   const std::vector<IndexInt>& collisions) {
            H_entry.rehash(M2);
